...
<CardGrid items={[{"title":"New Projects","icon":"rocket","content":"<p><strong>Use Custom API</strong> from day one for best performance and full feature access.</p>"},{"title":"AWS Migration","icon":"right-arrow","content":"<p><strong>Start with DynamoDB API</strong>, then migrate to Custom API over time.</p>"},{"title":"SQL Background","icon":"document","content":"<p><strong>Use SQL API</strong> for exploration, <strong>Custom API</strong> for production.</p>"}]} />
