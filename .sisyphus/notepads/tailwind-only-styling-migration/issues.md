...
- F1 audit rejected the migration: `src/content/docs/index.svx` still contains legacy semantic classes (`hero-*`, `btn-gradient`, `btn-outline`, `gradient-text`), runtime hooks still depend on `.ts-tabs-static-*` and `.heading-anchor`, and `src/lib/components/ApiExplorer.svelte` still contains the non-Tailwind `muted` class.
